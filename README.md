...
**Brain.** The model runs on your own graphics card through llama.cpp (MIT; the Vulkan build runs AMD, Intel and NVIDIA alike, where Ollama accelerates only a short list of AMD cards). `agent/brain/get-brain.bat` fetches the engine and Qwen3-4B-Instruct-2507 (Apache-2.0, Q4_K_M, ~2.5 GB): on the 30-sample harness it made the exact tool call 38% of the time against 26% for Qwen2.5-7B, at 3.7 s a turn against 12.7 on an 8 GB card, and it fits whole beside the voice. `get-brain.bat big` adds the 7B for 12 GB+ cards (`SKW_BRAIN_SIZE=big`). Tool calls are grammar-constrained by llama-server itself, a concrete order must open with a move, and a reply that comes back as JSON or a wall of text is rewritten once under a one-line grammar. Ollama stays a fallback (`SKW_BRAIN_BACKEND=off`); a Groq key stays optional for speed.
